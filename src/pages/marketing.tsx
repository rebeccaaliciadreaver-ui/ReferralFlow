...
interface Props { profile: UserProfile; initialEvents?: MarketingEvent[]; }

export default function MarketingPage({ profile, initialEvents = [] }: Props) {
  const [events, setEvents] = useState(initialEvents);
  const snapshots = useMemo(() => getKpiSnapshots(events, marketingKpis), [events]);
  const track = (event: string, channel?: MarketingEvent['channel']) => setEvents((current) => trackMarketingEvent(current, event, channel));
  ...
  return <main style={{ maxWidth: 1100, margin: '0 auto', padding: 32, color: '#eef2ff', background: '#101329', minHeight: '100vh', fontFamily: 'system-ui' }}>
    ...
    <p style={{ color: '#aab1d6', marginTop: 28 }}>Targets reset automatically at the start of each week. Events are passed to the analytics adapter so they can be persisted by the host application.</p>
  </main>;
}
